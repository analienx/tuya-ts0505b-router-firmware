...
__attribute__((weak)) void hb_board_apply_output(const hb_light_output_t *output)
{
  (void)output;
}
void hb_zcl_sync_output(uint8_t endpoint)
{
  ...
  if (read_u8(endpoint, ZCL_COLOR_CONTROL_CLUSTER_ID,
    ...
    if (color_mode == SL_ZIGBEE_ZCL_COLOR_MODE_CURRENT_HUE_AND_CURRENT_SATURATION
      ...
      hb_light_apply_hs(&state, hue, saturation);
      ...
      hb_light_apply_xy(&state, x, y);
    } else if (color_mode == SL_ZIGBEE_ZCL_COLOR_MODE_COLOR_TEMPERATURE
               && read_u16(endpoint, ZCL_COLOR_CONTROL_CLUSTER_ID,
                           ZCL_COLOR_CONTROL_COLOR_TEMPERATURE_ATTRIBUTE_ID,
                           &color_temperature)) {
      hb_light_apply_color_temperature(&state, color_temperature);
    }
  }

  hb_light_render(&state, &output);
  if (hb_light_state_is_consistent(&state, &output)) {
    hb_board_apply_output(&output);
  }
}
